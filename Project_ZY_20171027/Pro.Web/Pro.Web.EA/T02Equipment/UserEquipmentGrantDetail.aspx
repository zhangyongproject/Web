...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function LoadData(ret) {
            ...
            $j("#txtusers").val(rootNode.selectSingleNode("items/item/USERNAME").text);
            $j("#txtusers").data("userid", rootNode.selectSingleNode("items/item/USERID").text);
        ...
        var eiids, einames;
        function AddorEdit() {
            if (act != "editdate" && $j("#txtusers").val().trim().length == 0) { alert("请选择用户。"); return; }
            ...
            hashtable.add("userid", ($j("#txtusers").data("userid")));
            hashtable.add("username", ($j("#txtusers").val().trim().split("【")[0]));
            ...
            if (act == "editdate") { hashtable.remove("userid"); hashtable.remove("username"); hashtable.remove("eiid"); hashtable.remove("einame"); hashtable.remove("description"); }
        ...
        function SelectExamEquipment() {
            ...
            }, document.documentElement.clientWidth > 400 ? (document.documentElement.clientWidth - 100) : 400);
        }

        function SelectExamUser() {
            var xmlDoc = CreateXmlFromString(mXml.selectSingleNode("xml/users").xml.replaceAll("users", "xml").replaceAll("item", "data").replaceAll("key", "value").replaceAll("name", "data"));
            ShowSingleSelectWindow("请选择用户名", xmlDoc, $("txtusers").tag, function (value, text) {
                $j("#txtusers").val(text);
                $j("#txtusers").data("userid", value);
            }, document.documentElement.clientWidth > 400 ? (document.documentElement.clientWidth - 100) : 400);
        }
    </script>
</asp:Content>
...
<%--                <select id="sltuser" style="width: 200px">
                ...
                <input type="text" id="txtusers" style="width:100%;" value="" />
            </td>
                
            <td><input type="button" id="btnusers" value="..." onclick="SelectExamUser()" /></td>
        </tr>
        <tr id="trEquipment">
            ...
            <td><input type="button" id="btnequipments" value="..." onclick="SelectExamEquipment()" /></td>
        </tr>
        <tr>
            ...
            <td>
                <input type="text" id="txtbegintime" onfocus=" WdatePicker({ dateFmt: 'yyyy-MM-dd', onpicked: function () { $('txtendtime').focus(); }, maxDate: '#F{$dp.$D(\'txtendtime\')}' })" class="Wdate" style="width:100%;" />
            </td>
             <td>
                 -
             </td>
            <td>
                <input type="text" id="txtendtime" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd', minDate: '#F{$dp.$D(\'txtbegintime\')}' })" class="Wdate" style="width:100%;" />
            </td>
             <td></td>
<%--            <td>
