...
namespace Pro.Base.Common
{
    public  class ZipClass
    {
        ...
        public static void ZipFileDictory(string zipDic,string filename )//string[] args)
        {
            string[] filenames = Directory.GetFiles(zipDic);//args[0]);
            Crc32 crc = new Crc32();

            ZipOutputStream s = new ZipOutputStream(File.Create(filename));//args[1]));
            s.SetLevel(6);
            foreach (string file in filenames)
            {
               ...
               // string[] name = file.Split('\\'); 
                ZipEntry entry = new ZipEntry(string.Format(@"prologs\{0}", Path.GetFileName(file)));
